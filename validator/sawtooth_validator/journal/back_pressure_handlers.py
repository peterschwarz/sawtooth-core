...
class ClientBatchSubmitBackpressureHandler(Handler):
    ...
    def handle(self, connection_id, message_content):
        if not self._can_accept():
            LOGGER.debug('Applying back pressure to batch list received')
...
class GossipBatchBackpressureHandler(Handler):
    """This handler receives a batch and accepts it, if the system is able.
    ...
    def handle(self, connection_id, message_content):
        ...
        if gossip_message.content_type == GossipMessage.BATCH and \
                not self._can_accept():
            LOGGER.debug('Applying back pressure to batch received')
