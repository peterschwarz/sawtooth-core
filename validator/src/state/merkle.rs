...
impl MerkleDatabase {
    ...
    pub fn prune(db: &LmdbDatabase, merkle_root: &str) -> Result<bool, StateDatabaseError> {
        ...
        if change_log.get_successors().len() > 1 {
            ...
            // deleting a parent
            let successor = change_log.get_successors().first().unwrap();
...
/// the merkle root hash at the time of it's creation.
...
mod tests {
    ...
    fn merkle_trie_pruning_parent() {
        run_test(|merkle_path| {
            ...
            assert!(MerkleDatabase::prune(&db, &parent_root).expect("Prune should have no errors"));
    ...
    fn merkle_trie_pruinng_successors() {
        run_test(|merkle_path| {
            ...
            let mut parent_change_log = expect_change_log(&db, &parent_root_bytes);
            expect_change_log(&db, &successor_root_left_bytes);
